...
use crate::wire::{
    ...
    STATEMENT_ASSIGNMENT, STATEMENT_BLOCK, STATEMENT_CONTINUE, STATEMENT_FOR_LOOP,
...
pub fn run(request: &mut ffi::WireYulOptimizerRequest) -> Result<(), OptimizerError> {
    ...
    let mut eliminator = EqualStoreEliminator {
        request,
    ...
    eliminator.visit_block_root()?;
    eliminator.remove_pending_statements();
    Ok(())
}

struct EqualStoreEliminator<'a> {
...
impl EqualStoreEliminator<'_> {
    ...
    fn visit_block(&mut self, block_id: u64) -> Result<(), OptimizerError> {
        ...
        if self.variable_scopes.len() != scope_count {
            return Err(OptimizerError::InvalidWire(
                "scope stack mismatch in EqualStoreEliminator".to_string(),
            ));
        }
    ...
    fn visit_statement(&mut self, statement_id: u64) -> Result<(), OptimizerError> {
        if let Some(expression_id) = self.expression_statement_expression(statement_id) {
            if let Some((key, value)) = self.simple_store(StoreLocation::Storage, expression_id) {
                if self.environment.storage.get(&key).copied() == Some(value) {
                    self.pending_removals.insert(statement_id);
                }
            } else if let Some((key, value)) =
                self.simple_store(StoreLocation::Memory, expression_id)
            {
                if self.environment.memory.get(&key).copied() == Some(value) {
                    self.pending_removals.insert(statement_id);
                }
            }
        }
        ...
        match statement.kind {
            crate::wire::STATEMENT_EXPRESSION => {
                self.visit_expression_statement(statement.expression_id)
            }
    ...
    fn visit_for_loop(&mut self, statement: &ffi::WireStatement) -> Result<(), OptimizerError> {
        ...
        {
            return Err(OptimizerError::InvalidWire(
                "EqualStoreEliminator requires ForLoopInitRewriter.".to_string(),
            ));
        }
    ...
    fn handle_assignment(
        ...
        for name in &names {
            ...
            if !is_declaration {
                self.environment.storage.remove(name);
                self.environment.memory.remove(name);
                self.environment
                    .storage
                    .retain(|_, stored_value| stored_value != name);
                self.environment
                    .memory
                    .retain(|_, stored_value| stored_value != name);
            }
    ...
    fn expression_statement_expression(&self, statement_id: u64) -> Option<u64> {
        let statement = &self.request.statements[statement_id as usize];
        (statement.kind == crate::wire::STATEMENT_EXPRESSION).then_some(statement.expression_id)
    }
    ...
    fn remove_pending_statements(&mut self) {
        for block in &mut self.request.blocks {
            block
                .statement_ids
                .retain(|statement_id| !self.pending_removals.contains(statement_id));
        }
    }
}
